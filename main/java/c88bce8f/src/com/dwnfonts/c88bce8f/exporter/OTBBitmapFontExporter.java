package com.dwnfonts.c88bce8f.exporter;
...
import com.dwnfonts.c88bce8f.truetype.*;
...
public class OTBBitmapFontExporter implements BitmapFontExporter {
	...
	private static final TrueTypeFile createTrueTypeTables(BitmapFont bf, int xsize, int ysize, boolean extendWinMetrics) throws IOException {
		...
		makeCharacterGlyph(bf, bf.getNamedGlyph(".notdef"), ".notdef", a, xsize, ysize, entries, hmtxTable, postTable);
		makeCharacterGlyph(bf, bf.getCharacter(0x00), 0x00, a, xsize, ysize, entries, hmtxTable, postTable);
		makeCharacterGlyph(bf, bf.getCharacter(0x0D), 0x0D, a, xsize, ysize, entries, hmtxTable, postTable);
		makeCharacterGlyph(bf, bf.getCharacter(0x20), 0x20, a, xsize, ysize, entries, hmtxTable, postTable);
		for (Map.Entry<Integer,BitmapFontGlyph> e : bf.characters(false).entrySet()) {
			int cp = e.getKey(); if (cp == 0x00 || cp == 0x0D || cp == 0x20) continue;
			makeCharacterGlyph(bf, e.getValue(), e.getKey(), a, xsize, ysize, entries, hmtxTable, postTable);
		}
